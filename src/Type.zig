...
pub const zig = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, comptime T: type) !Type {
        ...
        const tag = try allocator.dupe(Tag, nodes.items(.tag));
    ...
    fn appendType(s: *InitState, comptime T: type) !Index {
        switch (@typeInfo(T)) {
            .Void => {
            ...
            .Int => |int| {
                const tag: Tag = switch (int.signedness) {
            ...
            .Float => |float| {
                const tag: Tag = switch (float.bits) {
                    8 => .f8,
                    16 => .f16,
                    32 => .f32,
                    64 => .f64,
    ...
    fn appendIdent(s: *InitState, comptime ident: []const u8) !Index {
        const index: Index = @intCast(s.idents.items.len);
    ...
    fn appendNode(s: *InitState, tag: Tag) !Index {
    ...
    inline fn setRhs(s: *const InitState, index: Index, rhs: u32) void {
    ...
    fn expectType(comptime T: type, root: ExpectedNode) !void {
        var ty = try init(std.testing.allocator, T);
        defer ty.deinit(std.testing.allocator);
        try expectNode(ty, 0, root);
    }

    fn expectNode(ty: Type, index: Index, expected: ExpectedNode) !void {
