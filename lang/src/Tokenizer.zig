...
pub fn next(t: *Tokenizer) Token {
    ...
    while (t.pos < t.src.len) : (t.consume()) {
        const c = t.src[t.pos];
        switch (state) {
            .start => switch (c) {
                '+' => {
    ...
    return Token{
        ...
        .loc = Token.Loc{
            .start = start,
            .end = end orelse t.pos,
        },
    };
}

inline fn consume(t: *Tokenizer) void {
    t.pos += 1;
}

inline fn prev(t: Tokenizer) u8 {
