...
test "ref view" {
    ...
    try std.testing.expectEqual(@bitCast(u64, ref1), view0.v1.id);
    try std.testing.expectEqual(@bitCast(u64, ref0), view1.v1.id);
}
...
test "struct view" {
    ...
    try std.testing.expectEqual(@bitCast(u64, expected.ref), view.v1.ref.id);
}
...
test "tuple view" {
    ...
    try std.testing.expectEqual(@bitCast(u64, expected[2]), view.v1[2].id);
}
...
test "union view" {
    ...
    expected = Expected{
        .Ref = @bitCast(super.ObjectId, @as(u64, 0)),
    };
    try index.add(0, expected);
    view = index.read(2);
    try std.testing.expectEqualDeep(@tagName(expected), @tagName(view.v1));
    try std.testing.expectEqualDeep(@bitCast(u64, expected.Ref), view.v1.Ref.id);
}
...
fn TestIndex(comptime types: anytype) type {
    return struct {
        ...
        fn getBytes(self: *Self, comptime _: []const u8, comptime _: []const u8, id: u64) ?[]const u8 {
            const obj_id = @bitCast(super.ObjectId, id);
...
fn writeFieldType(comptime info: definition.FieldType, value: anytype, writer: anytype) !usize {
    switch (info) {
        ...
        .Ref => {
            try serde.serialize(@bitCast(u64, value), writer);
            return @sizeOf(u64);
        },
        .Array => |array_info| {
...
fn writeSlice(comptime Child: type, slice: []const Child, writer: anytype) !usize {
    ...
    bytes.ptr = @ptrCast([*]const u8, @alignCast(1, slice.ptr));
